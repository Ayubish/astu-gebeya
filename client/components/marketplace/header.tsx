    ...
    <header className="sticky top-0 z-50 border-b border-border bg-card/90 backdrop-blur-md shadow-sm transition">
      <div className="container mx-auto px-4 py-2 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-3">
          {navItems.map((item) => (
            <Link key={item} href={item === "Home" ? "/" : item === "Categories" ? "/categories" : "/browse"}>
              <Button
                variant="ghost"
                className={`text-foreground hover:bg-accent/10 transition ${
                  getActiveNav() === item ? "underline underline-offset-4 font-semibold text-primary" : ""
                }`}
              >
                {item}
              </Button>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-2">
            <Link href="/auth/login">
              <Button variant="ghost" className="text-foreground hover:bg-accent/10 font-medium">
                Login
              </Button>
            </Link>
              ...
              <Link key={item} href={item === "Home" ? "/" : item === "Categories" ? "/categories" : "/browse"}>
                <Button
                  ...
                  className={`w-full justify-start text-left ${
                    getActiveNav() === item ? "bg-primary text-primary-foreground" : ""
                  }`}
                  onClick={() => {
